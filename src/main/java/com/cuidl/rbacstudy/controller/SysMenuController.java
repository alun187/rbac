...
//    }
    ...
    public R list(){
        // 查询所有菜单信息
        List<SysMenu> menuList=sysMenuService.list(new QueryWrapper<SysMenu>().orderByAsc("order_num").eq("del_flag", 0));
    ...
    public R delete(@PathVariable(value = "id")Long id){
        ...
        sysMenuService.update(new UpdateWrapper<SysMenu>().eq("id", id).set("del_flag", "1"));
        //redisUtil.removeByPrex(Constant.AUTHORITY_KEY);
        return R.ok();
    }
